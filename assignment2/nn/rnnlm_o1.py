...
class RNNLM(NNBase):
    ...
    def _acc_grads(self, xs, ys):
        ...
        dJdL = zeros(self.params.U.shape)
        ...
        for t in reversed(range(0, ns)):
            ...
            delta = (self.params.U.T.dot(delta3[t])) * (hs[t] * (1-hs[t]))
            # Gradient with respect to U (for this time step only)
            dJdU += outer(delta3[t], hs[t])
            for step in range(max(0, t-self.bptt), t+1):
                ts = t - step
                # print "Backprop for t=%d at step %d" % (t,  ts)
                dJdH += outer(delta, hs[ts-1])
                self.sgrads.L[xs[ts]] = delta
                # Update delta
                delta = self.params.H.T.dot(delta) * (hs[ts-1] * (1-hs[ts-1]))
